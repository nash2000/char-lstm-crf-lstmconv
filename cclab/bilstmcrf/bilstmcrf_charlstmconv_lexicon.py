...
datasets = { 'kornerdatasetlexicondb' : (kornerdatasetlexicondb.load_data, kornerdatasetlexicondb.prepare_data, kornerdatasetlexicondb.load_dict_data, kornerdatasetlexicondb.num_additional_feats) }
...
def init_params(options, embeddings=None):
    ...
    randn = numpy.random.rand(options['n_feats'],
                              options['dim_general_feat_proj'])
    params['Femb'] = (0.01 * randn).astype(config.floatX)
...
def adadelta(lr, tparams, grads, x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y, cost):
    """
    ...
    f_grad_shared = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y], cost, updates=zgup + rg2up,
...
def rmsprop(lr, tparams, grads, x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y, cost):
    """
    ...
    f_grad_shared = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y], cost,
...
def build_model(tparams, options):
    ...
    x_feat = tensor.tensor3('x_feat', dtype='int64')
    ...
    n_feats = x_feat.shape[2]           ## feattypes
    ...
    emb_feat =  tparams['Femb'][x_feat.flatten()].reshape([n_timesteps,
                                                n_samples,
                                                n_feats * options['dim_general_feat_proj']])
    ...
    emb = tensor.concatenate([emb_word, emb_pos, emb_chunk, emb_feat, emb_char_lstm, emb_char_conv], axis=2)
    ...
    def _crf_step(m_, obs_, y_, delta_, bscore_, bstate_, yprev_, ys_):
        ...
        #d_ = delta_.dimshuffle(0, 1, 'x')   #delta: n_samples * n_dim * n_dim
    ...
    #enda_ =    tparams['A'][:, endtagid].dimshuffle('x', 0)    # why it does not work?
    ...
    #transitionparams       =    tensor.tile( tparams['A'], [n_samples, 1, 1])
    ...
    #bval ==> n_timesteps * n_samples
    #beststates    = tensor.concatenate( [bval[::-1], [bestnewstates]])

    #beststates    = tensor.concatenate( [bval[::-1], [bestnewstates]])

    beststates = bval[::-1]

    f_pred_prob = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y], [yscores,deltas,bestnewscores,bestnewstates,pval[1],pred], name='f_pred_prob')
    #f_pred = theano.function([x, mask], pred.argmax(axis=1), name='f_pred')
    f_pred = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char], beststates, name='f_pred')

    return use_noise, x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y, f_pred_prob, f_pred, cost
...
def pred_probs(f_pred_prob, prepare_data, data, iterator, verbose=False):
    ...
    for _, valid_index in iterator:
        x, mask, x_feat, x_char, mask_char, y = prepare_data([data[0][t] for t in valid_index],
        ...
        pred_probs = f_pred_prob(x_word, x_pos, x_chunk, mask, x_feat, x_char, mask_char)
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False, valid_size=-1,  word_str_dict=None, tag_str_dict=None, predictto=None):
    ...
    for _, valid_index in iterator:
        ...
        preds = f_pred(x_word, x_pos, x_chunk, mask, x_feat,  x_char, mask_char)
...
def test_pred_error(f_pred, f_pred_prob, prepare_data, data, iterator, verbose=False, valid_size=-1):
    ...
    for _, valid_index in iterator:
        ...
        if 1 == 0:
            yscores,deltas,bscores,bstates,ballstates,pred = f_pred_prob(x,x_pos, x_chunk, mask, x_feat, y)
        ...
        preds = f_pred(x,x_pos, x_chunk, mask, x_feat, x_char, mask_char )
...
def train_lstm(
    ...
    dim_char_proj=25, # char embedding function
    ...
    #saveto='bilstmcrf_charlstmconv_general.npz',  # The best model will be saved there
    saveto='bilstmcrf_charlstmconv_lexicon.npz',  # The best model will be saved there
    predictto=None,
    savetodir=None,
    ...
    load_data, prepare_data, load_dict_data, num_additional_feats = get_dataset(dataset)
    ...
    n_chars = None
    n_syllfeats = None
    n_feattypes = num_additional_feats()

    word_dict,tag_dict, feat_dict, syllfeat_dict, char_dict  = dicts
    ...
    n_general_feats = numpy.max(syllfeat_dict.values()) + 1

    model_options['n_postags'] = n_feats[0]     # for postags
    model_options['n_chunks'] = n_feats[1]      # for chunks
    model_options['n_chars']  = n_chars
    model_options['n_feats']  = n_general_feats
    model_options['n_feattypes'] = n_feattypes

    print "n_postags", n_feats[0]
    print "n_chunks", n_feats[1]
    print "n_general_feats", n_general_feats
    print "n_feattypes", n_feattypes

    print "n_chars", n_chars


    dim_proj = dim_word_proj + dim_postag_proj + dim_chunk_proj + dim_general_feat_proj * n_feattypes + dim_char_proj * 2 + dim_conv_char_proj
    ...
    saveto = "bilstmcrf_charlstmconv_lexicon%d_%d_wordDim%d_charDim%d_ncontext%d.npz" % (dim_proj, dim_lstm_hidden_proj, dim_word_proj, dim_char_proj, n_char_context)
    ...
    (use_noise, x, x_pos, x_chunk, mask, x_feat, x_char, mask_char,
    ...
    f_cost = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y], cost, name='f_cost')

    grads = tensor.grad(cost, wrt=tparams.values())
    f_grad = theano.function([x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y], grads, name='f_grad')

    lr = tensor.scalar(name='lr')
    f_grad_shared, f_update = optimizer(lr, tparams, grads,
                                        x, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y, cost)
        ...
        try:
            for eidx in xrange(max_epochs):
                ...
                for _, train_index in kf:
                    ...
                    x, mask, x_feat, x_char, mask_char, y = prepare_data(x, y)
                    ...
                    cost = f_grad_shared(x_word, x_pos, x_chunk, mask, x_feat, x_char, mask_char, y)
                    f_update(lrate)
